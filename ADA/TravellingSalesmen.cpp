...
int calculateBound(vector<vector<int>>& graph, vector<bool>& visited, int currNode, int N) {
    int bound = 0;
    for (int i = 0; i < N; i++) {
        if (!visited[i]) {
            int minEdge = INF;
...
int tspBranchAndBound(vector<vector<int>>& graph, vector<bool>& visited, int currNode, int N, int cost, int& minCost) {
    ...
    for (int i = 0; i < N; i++) {
        if (!visited[i] && graph[currNode][i] != INF) {
            ...
            if (newCost + bound < minCost) {
                minCost = tspBranchAndBound(graph, visited, i, N, newCost, minCost);
            }
        }
    }
